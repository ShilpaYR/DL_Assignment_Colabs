# Deep Learning Assignment Colabs

This repository contains my executed Google Colab notebooks, archived with outputs, along with YouTube walkthrough videos explaining each notebook code block by code block.

The purpose of this assignment is to demonstrate a fundamental understanding of:
- deep learning basics
- tensor operations
- probability, linear algebra, and calculus foundations for deep learning
- basic neural network architecture
- implementation across NumPy, PyTorch, TensorFlow/Keras, and JAX

## Assignment completion summary

For this assignment, I:
- copied the Colab notebooks into my drive
- executed the notebooks
- archived the executed Colabs in this GitHub repository
- recorded one video per Colab topic, explaining the notebook block by block
- included YouTube video links in this README for easy access
- stored notebooks with inputs and outputs in the repository where available

## Repository contents

This repository includes executed notebooks such as:
- neural networks from scratch
- calculus for deep learning
- PyTorch neural networks tutorial
- Keras/TensorFlow neural networks tutorial
- JAX neural networks tutorial
- PyTorch tensors from zero to hero
- JAX deep learning tutorial
- Keras/TensorFlow advanced tutorial
- linear algebra for deep learning
- NumPy foundations for deep learning
- probability for deep learning
- probability fundamentals for deep learning
- PyTorch advanced tutorial
- TensorFlow tensor operations tutorial
- why deep learning works: geometric intuition

## Notebook and video mapping

Below is the exact mapping between the archived notebooks in this repository and the corresponding YouTube demo videos.

| Notebook in Repository | Topic | Video Demo |
|---|---|---|
| `Colab_1_neural_networks_from_scratch_EXECUTED.ipynb` | Neural Network from Scratch | [Neural Network from scratch](https://youtu.be/HkpazFqAMA4) |
| `calculus_for_deep_learning_FULLY_RUNNABLE_EXECUTED.ipynb` | Calculus for Deep Learning | [Calculus demo](https://youtu.be/O7bLXcFJ800) |
| `colab_2_pytorch_neural_networks_tutorial_EXECUTED.ipynb` | PyTorch Neural Networks Tutorial | [Colab2 pytorch neural network](https://youtu.be/LZjPB4qs_GQ) |
| `colab_3_keras_tensorflow_neural_networks_tutorial_EXECUTED.ipynb` | Keras/TensorFlow Neural Networks Tutorial | [Colab 3 keras tensorflow neural network](https://youtu.be/CMArCmLs7Nc) |
| `colab_4_jax_neural_networks_tutorial_CLEAN (1).ipynb` | JAX Neural Networks Tutorial | [Colab 4 jax video](https://youtu.be/M1fX_TgSUho) |
| `colab_4_pytorch_tensors_from_zero_to_hero_EXECUTED.ipynb` | PyTorch Tensors from Zero to Hero | [Recording 4 Pytorch 0 to hero 1](https://youtu.be/i2_uu7VbtNM) and [Recording 4 Pytorch 0 to hero 2](https://youtu.be/ifPOW8MqDRg) |
| `colab_5_jax_deep_learning_tutorial_EXECUTED.ipynb` | JAX Deep Learning Tutorial | [Colab 5 Jax Demo 1](https://youtu.be/z2i7ajPgC8U) and [Colab 5 Jax Demo 2](https://youtu.be/c8bF-w1SI4E) |
| `keras_tensorflow_advanced_tutorial_EXECUTED.ipynb` | Keras/TensorFlow Advanced Tutorial | [Keras tensorflow advanced](https://youtu.be/-hv0-xujGT8) |
| `linear_algebra_for_deep_learning_EXECUTED.ipynb` | Linear Algebra for Deep Learning | [Linear Algebra for Deep Learning](https://youtu.be/IbgRIT7R6Wc) |
| `numpy_foundations_for_deep_learning_EXECUTED.ipynb` | NumPy Foundations for Deep Learning | [Numpy foundation notebook record](https://youtu.be/X5kyJNDBsRA) |
| `probability_for_deep_learning_EXECUTED.ipynb` | Probability for Deep Learning | [Probability for Deep Learning](https://youtu.be/N6_cMFGjMhI) |
| `probability_fundamentals_for_deep_learning_EXECUTED.ipynb` | Probability Fundamentals for Deep Learning | [Probabiliy Fundamentals](https://youtu.be/vR3pYfE3ipA) |
| `pytorch_advanced_tutorial_EXECUTED.ipynb` | PyTorch Advanced Tutorial | [Pytorch advanced tutorial](https://youtu.be/5UCf-TxyIws) |
| `tensorflow_tensor_operations_tutorial_EXECUTED.ipynb` | TensorFlow Tensor Operations Tutorial | [tensorflow tensor operations tutorial](https://youtu.be/7us5YxrGSCA) |
| `why_deep_learning_works_geometric_intuition_EXECUTED.ipynb` | Why Deep Learning Works - Geometric Intuition | [Why deep learning works geometric intuition](https://youtu.be/Hqi2H-ipFbk) |

## What the videos explain

Each video explains the notebook in a step-by-step manner, including:
- what each code block is doing
- what concepts are being introduced
- how the tensor operations or neural network steps work
- what the output means
- how each notebook contributes to foundational deep learning understanding

## Key learning areas covered

### Mathematical foundations
- Probability fundamentals
- Probability for deep learning
- Linear algebra for deep learning
- Calculus for deep learning

### Programming foundations
- NumPy foundations
- TensorFlow tensor operations
- PyTorch tensors from zero to hero

### Neural network foundations
- Neural network from scratch
- PyTorch neural networks tutorial
- Keras/TensorFlow neural networks tutorial
- JAX neural networks tutorial
- JAX deep learning tutorial
- PyTorch advanced tutorial
- Keras/TensorFlow advanced tutorial
- Why deep learning works: geometric intuition

## How to review this submission

1. Open any executed notebook in this repository to review the code, inputs, and outputs.
2. Use the matching YouTube link from the table above to watch the block-by-block explanation.
3. Review how the notebooks progress from mathematical foundations to tensor operations and then to neural network architecture and frameworks.

## Repository link

GitHub Repository: [DL_Assignment_Colabs](https://github.com/ShilpaYR/DL_Assignment_Colabs)

## Note

This repository contains the executed Colab notebooks and the matching YouTube explanations required for the assignment. Some topics are explained across two video parts where the walkthrough was split for completeness.
